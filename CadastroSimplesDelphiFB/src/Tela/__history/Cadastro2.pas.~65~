...
type
  TFormCadastro2 = class(TForm)
    ...
    lbl_CEP: TLabel;
    Panel15: TPanel;
    lbl_NomEndereco: TLabel;
    Panel16: TPanel;
    lbl_CodUF: TLabel;
    Panel17: TPanel;
    lbl_NomBairro: TLabel;
    DBEditCEP: TDBEdit;
    DBEditNomEndereco: TDBEdit;
    DBEditNomBairro: TDBEdit;
    DBEditUF: TDBEdit;
    ...
    procedure cxGridtesteDBTableView1RowLayoutCustomDrawRowSelection(
      Sender: TcxGridTableView; ACanvas: TcxCanvas;
      AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean);
    procedure cxGridtesteDBTableView1CustomDrawCell(
  ...
  private
    ...
    procedure AdjustPanels(AMasterPanel: TPanel; Panels: array of TPanel; Percentages: array of Integer);
...
begin
  ...
  lbl_CEP.Font.Size := Round(NewFontSize * 1);
  DBEditCEP.Font.Size := Round(NewFontSize * 1);

  lbl_NomEndereco.Font.Size := Round(NewFontSize * 1);
  DBEditNomEndereco.Font.Size := Round(NewFontSize * 1);

  lbl_NomBairro.Font.Size := Round(NewFontSize * 1);
  DBEditNomBairro.Font.Size := Round(NewFontSize * 1);

  lbl_CodUF.Font.Size := Round(NewFontSize * 1);
  DBEditUF.Font.Size := Round(NewFontSize * 1);
...
begin
  ...
  Begin
    ...
    Panel13.Height := Round(ClientHeight * 0.12);
  End
  ...
  Begin
    ...
    Panel7.Height := Round(ClientHeight * 0.115);
    Panel13.Height := Round(ClientHeight * 0.115);
  End
  ...
  Begin
    ...
    Panel7.Height := Round(ClientHeight * 0.11);
    Panel13.Height := Round(ClientHeight * 0.11);
  End
  ...
  Begin
    ...
    Panel7.Height := Round(ClientHeight * 0.108);
    Panel13.Height := Round(ClientHeight * 0.108);
  End
  Else
  Begin
    Panel2.Width := Round(ClientWidth * 0.46);
    ...
    Panel5.Height := Round(ClientHeight * 0.08);
    Panel4.Height := Round(ClientHeight * 0.08);
    Panel7.Height := Round(ClientHeight * 0.104);
    Panel13.Height := Round(ClientHeight * 0.104);
  End;

end;

procedure TFormCadastro2.AdjustPanels(AMasterPanel: TPanel;
  Panels: array of TPanel; Percentages: array of Integer);
var
  i: Integer;
  TotalWidth, PanelHeight, PanelSpacing: Integer;
  TotalPercent, UsedWidth, CurrentLeft: Integer;
begin
  ...
  else if UsedWidth > TotalWidth then
    AMasterPanel.Width := TotalWidth;
end;
...
procedure TFormCadastro2.cxGridtesteDBTableView1CustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
begin
...
procedure TFormCadastro2.cxGridtesteDBTableView1RowLayoutCustomDrawRowSelection(
  Sender: TcxGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean);
begin
     ACanvas.Brush.Color := $00E57F1A;
  ACanvas.Font.Color := clWhite;
  ACanvas.Font.Style := [fsBold];

  // Preenche a linha com as configurações definidas
  AViewInfo.Paint(ACanvas);

  // Indica que a linha foi desenhada
  ADone := True;
end;
...
begin
  ...
  lbl_CEP.Margins.Left := NewMarginSize;
  lbl_CEP.Margins.Right := NewMarginSize;
  lbl_CEP.AlignWithMargins := True;

  DBEditCEP.Margins.Bottom := Round(NewMarginSize * 0.5);
  DBEditCEP.Margins.Left := NewMarginSize;
  DBEditCEP.Margins.Right := NewMarginSize;
  DBEditCEP.AlignWithMargins := True;

  lbl_NomEndereco.Margins.Left := NewMarginSize;
  lbl_NomEndereco.Margins.Right := NewMarginSize;
  lbl_NomEndereco.AlignWithMargins := True;

  DBEditNomEndereco.Margins.Bottom := Round(NewMarginSize * 0.5);
  DBEditNomEndereco.Margins.Left := NewMarginSize;
  DBEditNomEndereco.Margins.Right := NewMarginSize;
  DBEditNomEndereco.AlignWithMargins := True;

  lbl_NomBairro.Margins.Left := NewMarginSize;
  lbl_NomBairro.Margins.Right := NewMarginSize;
  lbl_NomBairro.AlignWithMargins := True;

  DBEditNomBairro.Margins.Bottom := Round(NewMarginSize * 0.5);
  DBEditNomBairro.Margins.Left := NewMarginSize;
  DBEditNomBairro.Margins.Right := NewMarginSize;
  DBEditNomBairro.AlignWithMargins := True;

  lbl_CodUF.Margins.Left := NewMarginSize;
  lbl_CodUF.Margins.Right := NewMarginSize;
  lbl_CodUF.AlignWithMargins := True;

  DBEditUF.Margins.Bottom := Round(NewMarginSize * 0.5);
  DBEditUF.Margins.Left := NewMarginSize;
  DBEditUF.Margins.Right := NewMarginSize;
  DBEditUF.AlignWithMargins := True;
...
begin
  ...
  AdjustPanels(Panel7, [Panel8, Panel10, Panel11, Panel12], [25, 25, 8, 42]);
  AdjustPanels(Panel13, [Panel14, Panel15, Panel16, Panel17], [25, 25, 25,25]);
